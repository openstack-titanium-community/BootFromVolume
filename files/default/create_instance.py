...
# DELL - Removing standalone volume class
"""
...
class VolumeOptions(workflows.Step):
    ...
    def contribute(self, data, context):
        ...
        return context
"""
# DELL END - Removing standalone volume class

class SetInstanceDetailsAction(workflows.Action):
    SOURCE_TYPE_CHOICES = (
        ("image_id", _("Image")),
        ("instance_snapshot_id", _("Snapshot")),
        # DELL - Boot from volume options
        ("volume_id", _("Boot from volume.")),
        ("volume_snapshot_id", _("Boot from volume snapshot "
                                 "(creates a new volume).")),
        # DELL END - Boot from volume options
    )
    ...
    # DELL - Moving Boot from volume options feature
    volume_id = forms.ChoiceField(label=_("Volume"), required=False)
    volume_snapshot_id = forms.ChoiceField(label=_("Volume Snapshot"),
                                           required=False)
    ...
    def populate_volume_id_choices(self, request, context):
        ...
        try:
            volumes = [v for v in cinder.volume_list(self.request)
                       if v.status == api.cinder.VOLUME_STATE_AVAILABLE and
                       v.bootable == 'true']
            volume_options.extend([self._get_volume_display_name(vol)
                                   for vol in volumes])
        except:
    ...
    def populate_volume_snapshot_id_choices(self, request, context):
        ...
        try:
            snapshots = cinder.volume_snapshot_list(self.request)
            # Dell - return a list of volumes
            volumes = [v for v in cinder.volume_list(self.request)
                       if v.status == api.cinder.VOLUME_STATE_AVAILABLE and
                       v.bootable =='true'] #and v.snapshot_id is not None]
            
            snapshots = [s for s in snapshots
                         if s.status == api.cinder.VOLUME_STATE_AVAILABLE]
            
            # list to contain snapshots that are bootable
            snapshots_bootable = []
            for s in snapshots:
                #if s.volume_id in volumes:
                #    snapshots_bootable.append(s)
                for v in volumes:
                    if s.volume_id == v.id:
                        snapshots_bootable.append(s)
            
            volume_options.extend([self._get_volume_display_name(snap)
                                   for snap in snapshots_bootable])
            # End Dell BFV change
        except:
            exceptions.handle(self.request,
                              _('Unable to retrieve list of volume '
                                'snapshots.'))
        return volume_options   
     # DELL END - Moving Boot from volume options feature
    ...
    def clean(self):
        ...
        # DELL - Stop multiple instance from Volumes
        #if volume_type and count > 1:
        if (source == 'volume_id' or source == 'volume_snapshot_id') and count > 1:
        # DELL END - Stop multiple instance from Volumes
...
class SetInstanceDetails(workflows.Step):
    ...
    # DELL - Adding Volume Contributions
                   ,"volume_type",
                   "volume_id",
                   "device_name",  # Can be None for an image.
                   "delete_on_terminate"
    # DELL END - Adding Volume Contributions
    )
    ...
    def contribute(self, data, context):
        ...
        if "source_type" in data:
            ...
            # DELL - TODO adding volume contribution definitions
            context['volume_id'] = data.get(data['source_type'], None)
            # DELL - END adding volume contribution definitions
...
class LaunchInstance(workflows.Workflow):
    ...
    def handle(self, request, context):
        ...
        LOG.info('DT007 - Determine volume mapping options')
        LOG.info('DT007 - context.get(source_type) = "%s"' % context.get('source_type'))
        
        # DELL - volume type no longer exists 
        # if context.get('volume_type', None):
        if context.get('source_type') == 'volume_id' or context.get('source_type') == 'volume_snapshot_id':
            # Empty the source_id if booting from volume
            context['source_id'] = ""
        # DELL END - volume type no longer exists

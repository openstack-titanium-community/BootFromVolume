...
horizon.addInitFunction(function () {
  ...
  function update_launch_source_displayed_fields (field) {
   ...
   // Dell - Boot from Volume
   var instance_source = $this.val(),
       $extra_fields = $("#id_delete_on_terminate, #id_device_name");
   
   if (instance_source === "volume_id" || instance_source === "volume_snapshot_id") {
      $extra_fields.closest(".control-group").show();
    } else {
      $extra_fields.closest(".control-group").hide();
    }
  
  // Reset the other controls when the Instance source is changed
  $("#id_image_id").val( $("#id_image_id option:first-child").val() );
  $("#id_instance_snapshot_id").val( $("#id_instance_snapshot_id option:first-child").val() );
  $("#id_volume_id").val( $("#id_volume_id option:first-child").val() );
  $("#id_volume_snapshot_id").val( $("#id_volume_snapshot_id option:first-child").val() );
  $('#id_delete_on_terminate').attr('checked', false)
  $('#id_device_name').val('vda');
  
  // Dell - End Boot from Volume
  }
